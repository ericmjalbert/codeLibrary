 ...
 void GraphZoomedPhasePlane(int eloop)
{
     ...
     FILE * phasePlane;
        ...
        fprintf(phasePlane, "set yrange[-2000:1000]\n");
        fprintf(phasePlane, "unset key\n");
        fprintf(phasePlane, "set arrow from 0,0 to 1,0 nohead lt 0\n");
        fprintf(phasePlane, "set output \"regPhasePlaneZoomed_epsilon10e-%d.pdf\"\n", eloop*2 -1);
...
void GraphPhasePlane(int eloop)
{
     ...
     FILE * phasePlane;
        ...
        fprintf(phasePlane, "set ylabel \"V\"\n");
        fprintf(phasePlane, "set xrange [0:1]\n");
        fprintf(phasePlane, "set yrange[%f:%f]\n", -c/DELTA/epsilonRegularize*1.1, c/DELTA/epsilonRegularize*.3);
        fprintf(phasePlane, "unset key\n");
        fprintf(phasePlane, "set arrow from 0,0 to 1,0 nohead lt 0\n");
        fprintf(phasePlane, "set output \"regPhasePlane_epsilon10e-%d.pdf\"\n", eloop*2 -1);
        fprintf(phasePlane, "set multiplot\n");
        for(j = 0; j <= NUMTRAJ; j++)
        {
...
for(epsilonRegularize = 0.1; epsilonRegularize > 0.000001; epsilonRegularize*=0.01)
{
for(i = 0; i <= 1; i++){
printf("calculating backwards integrations\n");
    for(j = 0; j <= NUMTRAJ; j++)
    {
        ...
        NV_Ith_S(y, 1) = -c/DELTA/epsilonRegularize*.05 + c/DELTA/epsilonRegularize*.1*i;
                NV_Ith_S(y, 1) = -100000 + 100500*i;    
 ...
 */
 printf("calculating forward trajectories\n");
    ...
    for(j = 0; j <= NUMTRAJ; j++)
    {
        ...
        y = N_VNew_Serial(NEQ);
        NV_Ith_S(y, 0) = INITU*j - INITU / 2;
        NV_Ith_S(y, 1) = INITV*(j-3) - INITU / 2;
        NV_Ith_S(y, 1) = -c/DELTA/epsilonRegularize*.05 + c/DELTA/epsilonRegularize*.1*i;
                NV_Ith_S(y, 1) = -100000 + 100500*i;    
 ...
 */
 ...
 y = NULL;
        ...
        /* Setting vector to initial values */
        y = N_VNew_Serial(NEQ);
        NV_Ith_S(y, 0) = 0.00000000000000001;
